...
exports.DEFAULT_CONFIG_NAME = 'Forza 5';
...
function processGamepadConfig(config) {
    ...
    Object.keys(config).forEach((button) => {
        ...
        for (const code of codes) {
            if (code === 'Escape') {
                invalidButtons[button] = 'ESC Tuşunu hiçbir tuşa atayamazsınız.';
                continue;
            }
            if (codeMapping[code]) {
                invalidButtons[button] = `'${code}' Butonu zaten şurada kullanımda '${(0, formattingUtils_1.camelToSpace)(codeMapping[code].button)}'`;
                continue;
            }
...
function validateMouseConfig(mouseConfig) {
    ...
    if (mouseControls !== undefined && mouseControls !== 0 && mouseControls !== 1) {
        errors.mouseControls = 'Hatalı Çubuk Numarası';
    }
    if (sensitivity < 1 || sensitivity > 1000) {
        errors.mouseControls = 'Hassasiyet 1-100 arasında olmalı.';
    }
...
exports.defaultGamepadConfig = {
    ...
    keyConfig: {
        ...
        leftTrigger: ['RightClick', 'KeyS'],
        rightShoulder: 'KeyQ',
        rightTrigger: ['Click', 'KeyW'],
        start: 'Enter',
        select: 'Tab',
        home: undefined,
        dpadUp: ['ArrowUp', 'KeyX'],
...
chrome.runtime.onInstalled.addListener(({ reason }) => {
    ...
    if (typeof chrome.runtime.setUninstallURL === 'function') {
        chrome.runtime.setUninstallURL('https://watifu.com');
    }
});
